    ...
    window.opspark.makeGame = function() {
        ...
        */
        function update() {
            ...
            for (var i = 0; i < circles.length; i++) {
                circles[i].x += 1; // Move each circle one unit to the right
                circles[i].y += 1; // Move each circle one unit down
              }
        for (var i = 0; i < circles.length; i++) {
        ...
        */
        game.checkCirclePosition = function(circle) {
            ...
            if (circle.x < 0){
                circle.x = canvas.width; // Reposition on right edge
            }
            if (circle.y > canvas.height) {
                circle.y = 0; // Reposition to the top edge
            }
            if (circle.y < 0) {
                circle.y = canvas.height; //Reposition on the bottom edge
            }
